...
name: claude-review
description:
  "Ask Claude to review Shannon work. Use when the user requests Claude review
  or during large/risky experiments before acting on conclusions."
---

# Claude Review

Use Claude as an independent reviewer for Shannon issue designs, code diffs,
experiment results, and architectural decisions.

The review is advisory. Codex remains responsible for deciding whether Claude's
feedback is correct, acting on valid findings, and explaining any rejected
findings.
...
- an experiment is large, risky, or touches the Nushell/Reedline fork surface;
- an experiment has already failed once and the next design needs review;
- a code change affects architecture, shell state synchronization, process
  handling, signal handling, config loading, terminal behavior, or persistent
  state;
